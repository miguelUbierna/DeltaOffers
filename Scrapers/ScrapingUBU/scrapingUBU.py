    ...
    for item in etiqueta_pagina:
        ...
        if respuesta_subpagina.status_code == 200:
            ...
            for contenedor in contenedores_subpagina:
                descripcion = contenedor.find(
                    'div', class_='field-item even', property='content:encoded').p.get_text(strip=True)
                ...
                if str(contenedor.find('div', class_='field field-name-field-presentation-date field-type-datetime field-label-above').find('span')['class']) != "['date-display-single']":
                    ...
                    fecha_fin_solicitud = contenedor.find(
                        'div', class_='field field-name-field-presentation-date field-type-datetime field-label-above').find('span', class_='date-display-single')['content']
                    list_fin.append(fecha_fin_solicitud)
                ...
                if contenedor.find('div', class_='field field-name-field-receiver field-type-text-long field-label-above') != None:
                    contenedor_destinatarios = contenedor.find(
                        'div', class_='field field-name-field-receiver field-type-text-long field-label-above').find('div', class_='field-item even').get_text(strip=True)
